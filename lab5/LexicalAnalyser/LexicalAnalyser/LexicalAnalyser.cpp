...
void LexicalAnalyser::Analyse() {
    ...
    for(pair<int,string>& line: fileLines){
        ...
        for(char& ch : line.second){
            ...
            if (isVariable) {
                if(isdigit(word[0])){
                    errorsTable.insert(make_pair("test.java:"+to_string(line.first) + ":" + to_string(line.second.find(ch)-word.length()+1),"Variable mustnt starts with a digit!"));
                } else {
...
//                                [&word](const pair<string, string> &pr) { return word == pr.first; })) {
                        if (!any_of(keyWordsPattern.begin(), keyWordsPattern.end(),
                            ...
                            !any_of(variablesPattern.begin(), variablesPattern.end(),
                                    [&word](const pair<string, string> &pr) { return word == pr.first; })) {
                            variablesTable.insert(make_pair(word, "is a variable of "+variableType+" type"));
                            tokens.emplace_back(make_shared<Token>(variableType, word));
                            if(isChars && ch != ' '){
                                tokens.emplace_back(make_shared<Token>("Special symbol", string(1,ch)));
                            }
                        } else {
                            ...
                            errorsTable.insert(make_pair("test.java:" + to_string(line.first) + ":" +
...
//                    }
            ...
            if(word == "true" || word == "false"){
                ...
                pair<string,string> pr = make_pair(word,"boolean");
            ...
            errorsTable.insert(make_pair("test.java:"+to_string(line.first) + ":" + to_string(line.second.find(ch)-word.length()+1),"Unknown variable!"));
            word.clear();
        }
    }
